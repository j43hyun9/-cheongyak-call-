...
- ✅ RAG 제거(PR#7)·local 엔진(PR#8)·전처리 스크립트(PR#9) 머지 완료. ✅ **어투 데이터셋 완료(2026-07-06)**: 소스 200건(`data/colbi_sft_seed`+`_expand`), held-out 제외 후 train 168/val 19.
- 다음(플랜 ~7/15): ①~~데이터셋~~✅완료 ②**QLoRA 학습(전재형)** ← 지금 여기 ③GGUF→Ollama 서빙 연결(임강) ④프론트 최종 연동(백승옥)

## 팀원 담당 인계점 (2026-07-06 갱신 — 데이터셋 담당 전재형으로 변경)
- **전재형(PM)** — **어투 데이터셋 담당**(완료: `data/colbi_sft_*.jsonl` 200건) + **QLoRA 학습 주도**(Colab, Unsloth+Qwen2.5-7B) + 파인튜닝 **전/후 평가 리포트**(평가셋 30 held-out) + 조율·발표·문서. CLAUDE.md 단독 관리.
- **장두호** — (데이터셋 담당 → 전재형으로 이관) **어투 데이터 검수 + `colbi.py` 페르소나 유지**. 신규 집중 과제는 팀장과 조율(재배정 예정).
