...
if not OPENAI_API_KEY:
    raise ValueError(".env에서 OPENAI_API_KEY를 찾지 못했습니다. .env 변수명을 확인해주세요.")
...
def view_schedule_handler(date_str: str):    
    ...
    if dt is None:
        return (
            "날짜 형식을 확인해 주세요.\n"
            "예: 2026-06-05, 2026/06/05, 20260605"
        )
    ...
    return "\n".join(
        f"- {i['title']} ({i['datetime'][11:16]})"
        for i in items
    )

def view_deleted_schedule_handler():
    items = load_deleted_schedule()
    ...
    return "\n".join(
        f"- {i['title']} ({i['datetime'][:10]} {i['datetime'][11:16]})"
        for i in items
    )
...
DELETED_SCHEDULE_FILE = "deleted_schedule.json"
...
def _load_deleted() -> list:
    if not os.path.exists(DELETED_SCHEDULE_FILE):
        return []
    with open(DELETED_SCHEDULE_FILE, "r", encoding="utf-8") as f:
        return json.load(f)


def _save_deleted(data: list) -> None:
    with open(DELETED_SCHEDULE_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)


def delete_schedule(title: str, target_date: datetime) -> int:
    data = _load_all()
    deleted_data = _load_deleted()
    ...
    for item in data:
        same_date = item["datetime"].startswith(target_str)
        same_title = title in item["title"] or item["title"] in title
    ...
    _save_all(remaining)
    _save_deleted(deleted_data)

    return deleted_count


def load_deleted_schedule() -> list:
...
def extract_title(text):
    ...
    # 문장 끝 요청어 제거
    title = re.sub(
        r'(해|줘|주세요|해줘|해주세요|해줄래|해줄래요|부탁해|부탁해요|부탁드립니다)$',
        '',
        title
    )
...
def process_text_command(recognized_text):
    ...
    if any(k in recognized_text for k in unsupported_keywords):
        return "미지원 기능입니다. 현재는 일정 등록, 조회, 삭제만 가능합니다."

    intent_result = classify_intent(recognized_text)
    intent = intent_result["intent"]

    if intent == INTENT_IPO:
        if any(k in recognized_text for k in ["알림", "구독", "설정"]):
            answer = "아직 지원하지 않는 명령이에요."
            return answer

        items = fetch_ipo()
        today = datetime.today().strftime("%Y-%m-%d")

        if "오늘" in recognized_text:
            items = ipo_today(items, today)

        elif "이번주" in recognized_text or "이번 주" in recognized_text:
            items = ipo_week(items, today)

        elif "다음주" in recognized_text or "다음 주" in recognized_text:
            items = ipo_next_week(items, today)

        if not items:
            answer = "해당 기간에 공모주 청약 일정이 없습니다."
        else:
            # 청약 시작일 빠른 순서대로 정렬
            items = sorted(
                items,
            ...
            for item in items:
                ...
                start_text = f"{start_dt.month}월 {start_dt.day}일"
                end_text = f"{end_dt.month}월 {end_dt.day}일"

                answer += (
                    f"{start_text} {item['name']} 청약 시작\n"
                    f"청약기간 : {start_text} ~ {end_text}\n"
        ...
        deleted_count = delete_schedule(
            result["title"],
            result["datetime"]
        )

        if   deleted_count:
            answer = f"{result['title']} 일정이 삭제되었습니다."
        else:
            answer = f"{result['title']} 일정을 찾지 못했습니다."       
...
""")
    ...
    with gr.Tab("삭제된 일정"):
        gr.Markdown("### 🗑 삭제된 일정 확인")

        deleted_btn = gr.Button("🗑 삭제된 일정 조회")

        deleted_result = gr.Textbox(
            label="삭제된 일정 목록",
            interactive=False,
            lines=7,
            placeholder="삭제된 일정이 표시됩니다."
        )

        deleted_btn.click(
            fn=view_deleted_schedule_handler,
            inputs=None,
            outputs=deleted_result
        )
    # =========================
    # 이벤트 연결
    # =========================
